...
###############################################################################################
	...
	Program is to compare the upper confidence bound selection methods in a 10-armed bandit testbed, presented
...
class Agent_epsilon(object):
	...
	def __init__(self,nArms, eProb=0):
		...
		self.valEstimates = np.zeros(nArms)     # action value estimates sum(rewards)/Amount
		                                        #initializing with optimal value
	...
	def interpreter(self, reward,timestep):
...
class Agent_ucb(object):
	...
	def __init__(self,nArms, eProb=0):
		...
		self.timeStep_main = 0                    # Time Step t
		self.lastAction = None               # Store last action

		self.kAction = np.zeros(nArms)          # count of actions taken at time t
		self.rSum = np.zeros(nArms)             # Sums number of rewards
		self.valEstimates = np.zeros(nArms) 	# action value estimates sum(rewards)/Amount
		self.ucb_val_Estimates=np.zeros(nArms)    # upper-confidence-bound
		self.c=5								#constant for ucb
	...
	def action(self):
		...
		# Greedy Method
			...
			maxAction = np.argmax(self.ucb_val_Estimates)     # Find max ucb value estimate
			# identify the corresponding action, as array containing only actions with max
			action = np.where(self.ucb_val_Estimates == np.argmax(self.ucb_val_Estimates))[0]
	...
	def interpreter(self, reward,timestep):
		...
		# Increase time step
		self.timeStep += 1
		estimate=self.c*((np.log(self.timeStep_main)/self.kAction[At])**0.5)
		self.ucb_val_Estimates=self.valEstimates+estimate
		...
		for iIter in tqdm(range(self.iterations)):
			...
			for jPlays in range(self.plays):
				...
				for kAgent in self.agents:
					...
					# Agent checks state
					kAgent.interpreter(reward=rewardT,timestep=jPlays+1)
...
if __name__ == "__main__":
	...
	agents = [Agent_ucb(nArms=nArms,eProb=0),Agent_epsilon(nArms=nArms,eProb=0.1)]
